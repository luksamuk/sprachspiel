...
- **Tool Trait + Proc Macro `#[sprachspiel::tool]` (Issue #118)** — New `Tool` trait (`src/tools/tool_trait.rs`) and `#[sprachspiel::tool]` proc-macro crate (`sprachspiel-tool-derive/`) replace `ollama_rs::generation::tools::Tool` and `#[ollama_rs::function]` as the foundation for multi-provider support. The macro generates a `Params` struct (with `serde::Deserialize` + `schemars::JsonSchema` derives) and a `Tool` impl, working just like the previous ollama-rs version but with our own types. The `Tool` trait is now available at `crate::tools::Tool` and the macro at `sprachspiel_tool_derive::tool`. **In this PR, no existing tools are migrated** — that work happens incrementally in subsequent PRs. The 36 existing `#[ollama_rs::function]` tools continue to use the ollama-rs macro until each is migrated. Migration is a 1-line attribute change per tool (`#[ollama_rs::function]` → `#[sprachspiel::tool]`) once the coordinator supports the new trait.
