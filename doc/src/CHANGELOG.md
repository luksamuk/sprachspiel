...
- **`/togglestyle` command** — Toggle style rendering on/off with a single command. When style is off: Mermaid blocks show as source code blocks (no diagram rendering), code block syntax highlighting (syntect fg colors) is stripped (plain text with Catppuccin background preserved), and tables use pipe-delimited plain format (`| col | col |`) instead of box-drawing borders. Status bar indicator: 🎨 (style on) / 📄 (style off). Mental model: "I want to see the code underneath". `App.style_enabled` field, `toggle_style()` method, threaded through `render()` → `build_lines()` → `render_markdown_impl()`. 8 new tests.
