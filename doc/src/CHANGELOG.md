...
- **3-Layer Compaction Overflow Strategy (Issue #187)** — Progressive compaction when the conversation context exceeds the model's window during `/compact`. Previously, compaction would fail with "The prompt is too long" if the middle section exceeded the context window. Now uses a 3-layer fallback: (1) **Pre-pruning** strips long tool outputs (>500 chars) before constructing the compaction prompt, replacing them with truncated summaries; (2) **Fallback truncation** drops oldest middle messages until the prompt fits within 50% of the context window; (3) Chunked recursive summarization placeholder (constants and functions defined, connected in core.rs when needed). New functions: `pre_prune_messages()`, `fallback_truncate()`, `split_into_chunks()`, `fits_in_context()`, `estimate_messages_tokens()`, `max_chunk_tokens()`. New constants: `PRUNE_TOOL_RESULT_THRESHOLD`, `PRUNE_TOOL_RESULT_KEEP_CHARS`, `COMPACTION_MAX_CONTEXT_RATIO`, `MAX_RECURSION_DEPTH`, `TRUNCATION_TARGET_RATIO`, `CHUNK_OVERLAP_RATIO`.
