...
- **Retry Threshold with Backoff — Recoverable Server Errors (Issue #116)** — Classify LLM errors into per-category retry strategies with exponential backoff, instead of the current 0ms backoff with all-or-nothing recovery. New module `src/retry.rs` defines `RetryCategory` (`ImmediateRetry`, `NetworkRetry`, `ServerRetry`, `RateLimitRetry`, `NoRetry`) with per-category max attempts and `retry_delay()` for backoff calculation. Server errors (HTTP 500, OOM, cold start) now retry 3× with 5s/10s/15s backoff. Network errors (timeouts, connection refused) retry 5× with exponential backoff (100ms→200ms→400ms→800ms→1.6s). Tool execution failures (`InternalToolError`) are now **recoverable** — the error message is pushed to the conversation as a tool result, allowing the LLM to self-correct within the same turn instead of aborting the entire conversation. JSON parse errors remain non-retryable. New `src/chat/recovery.rs` wrapper centralizes the `ChatMessage::tool()` call site for the recovery pattern (single migration point in #121). `RateLimitRetry` variant is pre-emptive support for #122 (OpenAI-compatible provider) — `Retry-After` header support will be wired in #122. **Cancel-aware sleep:** retry backoff uses `tokio::select!` with the cancel token, so Ctrl+C during a 15s server-retry backoff aborts immediately instead of waiting for the timer. UI shows `"Retrying in Xs..."` on the first retry attempt. Query mode (`executor.rs`) uses the same classification. **W2 Wave Context:** This is the foundation for the W2 Provider Chain (#116→#123). The `src/retry.rs` module will be relocated to `src/provider/retry.rs` in #119 (Agnostic Provider Types), and `classify_for_retry(&OllamaError)` will be reimplemented to accept `ProviderError`. Within the W2 mini-sprint, the `#[allow(dead_code)]` policy is relaxed: code prepared for W2 future use (e.g., `RateLimitRetry` field `retry_after`) is tolerable as long as it is resolved by the W2 closure (#123). Prerequisite for #118, #119, #120, #121, #122, #11, #123.
