...
- **Concurrent reindex guard** — `ChatSession.is_reindexing` (`Arc<AtomicBool>`) prevents running `/reindex --yes` while another is already in progress. If a reindex is running, the command returns "Embedding reindex is already in progress. Please wait for it to finish." instead of starting a second one.
