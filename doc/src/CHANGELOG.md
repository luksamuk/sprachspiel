...
- **Tool Trait + Proc Macro `#[sprachspiel::tool]`** — New `Tool` trait and `#[sprachspiel::tool]` proc macro replace `ollama_rs::generation::tools::Tool` and `#[ollama_rs::function]`. The macro generates a `Params` struct (with `serde::Deserialize` + `schemars::JsonSchema` derives) and a `Tool` impl, working just like the previous ollama-rs version but with our own types. This is the foundation for multi-provider support: when #120 (OllamaProvider reqwest direct) lands, the tool system already speaks the project's own trait, removing the tightest coupling surface. During migration, both `#[ollama_rs::function]` and `#[sprachspiel::tool]` coexist — this PR migrates a representative subset (calc + system + weather + todos) and documents the migration pattern for the remaining tools.
