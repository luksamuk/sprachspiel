...
- **Compaction output visual separation** — Compaction summary now displays with a horizontal rule separator and completion count before and after the summary, instead of the previous `--- Compaction Summary ---` / `---------------` header/footer that blended into the markdown content. Chunk progress (`⚙ Compacting in 3 chunks...`) and truncation warnings (`⚠ Truncation applied: dropped 150 oldest messages`) now appear as separate system messages instead of being mixed into the streaming content.
