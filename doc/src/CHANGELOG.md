...
- **TUI event loop: fix idle CPU burn** — Changed `event::poll(Duration::from_millis(0))` to `event::poll(Duration::from_millis(SPINNER_TICK_MS))` in the async event loop. The zero-millisecond poll caused busy-wait at ~4300 iterations/second in idle state, consuming ~5% CPU. Blocking for up to 120ms (the spinner tick interval) idles the CPU properly while still waking up reliably for spinner animation (~8 fps) and key input.
