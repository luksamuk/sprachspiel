...
- **Fix Line.style propagation in Markdown rendering** — `tui-markdown` renders headings with heading style (color, bold, underline) in `Line.style` as a fallback for `Span`s with `Style::default()`. Three code paths were discarding `Line.style` when reconstructing `Line`s from `Span`s: (1) `wrap_styled_line()` in `wrap.rs` now propagates `Line.style` to each `Span` via `base_style.patch(span.style)` before processing, and `line_to_owned()` now preserves `Line.style`; (2) Tool message rendering in `chat_area.rs` now merges `Line.style` into each `Span` before applying the dim overlay via `base_style.patch(span.style).patch(dim_style)`; (3) Thinking block rendering is fixed by the `wrap_styled_line()` change. Without this fix, heading styles (including the new underline) were silently lost in Thinking blocks, Tool messages, and wrapped long headings.
