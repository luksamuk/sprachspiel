...
- **Completion menu: never auto-replaces text** — The auto-trigger completion on typing `/` only shows/hides the menu without replacing text. Tab or Enter must be used to confirm a selection. This prevents the stuck input bug where the menu would auto-replace partial input.
