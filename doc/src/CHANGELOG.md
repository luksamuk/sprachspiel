...
- **Catppuccin palette for TUI code blocks** — Code blocks (fenced) and inline code in the TUI now use colors from the [Catppuccin](https://catppuccin.com) palette (MIT License). Dark theme uses Mocha Surface0 background (`#313244`) with Mocha Text foreground (`#cdd6f4`). Light theme uses Latte Surface0 background (`#ccd0da`) with Latte Text foreground (`#4c4f69`). Monochrome theme uses Mocha Surface1 background (`#45475a`) with Mocha Subtext1 foreground (`#bac2de`) plus bold. Inline code uses the same fg/bg pairing without background. The `apply_code_block_background()` post-processor applies Catppuccin backgrounds and trailing padding to fenced code block lines, extending the background to the right edge of the chat area so code blocks appear as solid colored rectangles. This is necessary because `tui-markdown`'s syntax highlighter (syntect with `base16-ocean.dark` theme) sets foreground colors per token but never sets background colors. Clipboard copies are protected via `trim_end()` on `visual_lines`, stripping trailing padding whitespace while preserving code content. Credit: Catppuccin palette licensed under MIT.
