...
- **Cycle-aware message ordering in multi-round tool calls (Issue #201)** — Fixed UX regression where tool calls and their results from multiple rounds appeared in a single block at the end of the chat history, with the model's thinking/text content emitted first. In a multi-round cycle (e.g., model searches → observes results → searches again → final response), all tool indicators were batched after all text content, making it impossible to follow the model's reasoning. Root cause: the Ollama API streams thinking/content tokens in real-time but aggregates tool_calls into a single `done=true` chunk per round; the TUI event loop had no mechanism to group messages by round. Fix: `ChatMessage` now carries an ephemeral `round_index: usize` (not persisted to SQLite, reset per user prompt) that tracks which round of a multi-round cycle each message belongs to. A new `insert_at_round_boundary()` method positions inter-round content (thinking, text) after all messages of the previous round and before subsequent round messages. `App.current_round` tracks the round counter, incremented on `ToolCallStarted` and `InterToolText`, reset on `Complete`/new prompt. The coordinator, core, and persistence layers are unchanged — this is purely a TUI event-loop fix.
