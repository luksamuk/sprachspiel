...
- **Fix Ctrl+Y paste — add diagnostic logging** — Ctrl+Y (paste from system clipboard) now logs debug messages when the clipboard is empty or returns an error, making it easier to diagnose clipboard issues across different environments (SSH, Termux, etc.). Previously, errors were silently swallowed.
- **Improve Ctrl+Shift+C copy diagnosis** — Added `log::debug!()` when chat selection is active but `extract_text()` returns empty, helping diagnose cases where `visual_lines_cache` is stale or empty at copy time.
