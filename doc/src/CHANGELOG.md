...
- **CLI help text named the pre-rename binary `ask`, and was unreachable (LUC-142)** — Three defects in one, all found by reading the code rather than the issue text. (1) **The rich help blocks were never printed.** Every subcommand's `long_about` was declared as an attribute on its args struct, but a doc-comment on the `Subcommand` *variant* takes precedence over the struct's help attributes — clap compiled those blocks into the binary and emitted the one-line doc-comment instead, for `-h`, `--help` and `help <sub>` alike. The `long_about` now lives on the variant, so `sprach <sub> --help` prints it; `-h` keeps the short form, and the top-level command list is unchanged. The issue's own correction comment concluded the strings were "not user-visible" — that was checked only against the help paths, so it missed (2): **`sprach ocr` with no arguments printed `Usage: ask ocr …` to stderr.** That is a runtime error string, not help text, and it did reach users. Its `validate()` now names the binary via `APP_NAME`. (3) **`{app}` replaces the hardcoded name**, rendered by the new `consts::app::help_text()` — a single source of truth, so the next rename cannot leave the help behind. `concat!` was the obvious tool but cannot be used: it requires a literal and rejects a `const`. Help examples were also corrected while making them visible, since printing stale text is how LUC-140 happened: `ask ocr --table`/`--formula` never existed (the flag is `--mode table|formula|figure|text`), and `ask chat -m lfm` advertised a model that does not exist (`DEFAULT_MODEL` is `qwen3.5:4b`). Removed `SummarizeArgs::validate()` — dead code (`#[allow(dead_code)]`, no caller; `handle_summarize` reads stdin itself), and the last place the old name survived. Sensors: three unit tests in `ocr/cli.rs` (the runtime error names the real binary, the rendered `long_about` substitutes `{app}`), verified by re-injecting each defect.

### Removed

- **Attention Priming (Chunk Reordering) dropped — its justification was inverted (LUC-144, ex gh#172/#228 audit)** — The roadmap item was to reorder retrieved chunks so the top-scoring passages sit at the beginning and end of context, and it rested on `Cuconasu et al. 2025` cited as *support*. Reading that paper shows it says the **opposite**: "sophisticated strategies that attempt to rearrange the passages based on LLM positional preferences **do not perform better than random shuffling**." It tests this exact strategy (`MaxRelevance`) against a `Shuffle` baseline and finds no statistically significant difference (Wilcoxon, p=0.05). The citation had been written from the paper's title rather than from the paper — the same defect class as the bibliographic audit (LUC-143). The decision is recorded as **D-12** in `doc/src/development/research-icebox.md`, with the reasons: (1) the remaining delta was precisely the strategy the paper refutes — section-level ordering already exists in `build_context()`; (2) the paper's actual mechanism is not position but **hard distractors** — it finds that retrievers "systematically bring highly distracting ones to the top ranks, with over 60% of queries containing at least one highly distracting passage among the top-10", and that relevant and distracting passages penalise each other; our retrieval already fuses with RRF; (3) keeping the item would preserve the reasoning error it was filed for. The *live* half — hard-distractor robustness — is not scheduled; it overlaps Semantic Deduplication (LUC-119) and Metadata Enrichment (LUC-118) and would need an evaluation harness to be measured honestly. The *attention-based* half (**R-04**) survives on its own terms, deferred because Ollama does not expose attention weights. The hallucinated-citation record, which the `IMPLEMENTATION.md` consolidation had silently deleted, is restored in D-12 and in `context-anatomy.md`.

### Fixed

- **`Liu et al.` citation corrected to 2024 (TACL 12:157–173)** — The "Lost in the Middle" paper was cited as 2023 throughout (the arXiv preprint year). It was published in *Transactions of the Association for Computational Linguistics*, volume 12, pp. 157–173, 2024, DOI `10.1162/tacl_a_00638`. Corrected in the BibTeX entry (`papers-reference.md`, key now `liu2024lostinmiddle`, with volume/pages/DOI added), `context-anatomy.md`, `context_composition_design.md` (2×) and `implementation-history.md`. `context-anatomy.md` also gained the **methodological caveat** Liu et al.'s figure came from a controlled setting — rotating a single relevant passage among otherwise-irrelevant ones — which Cuconasu et al. describe as an "artificial configuration" that "amplifies the impact of the positional bias". The practical consequence is narrow and now stated where the claim is made: keep relevant content out of the middle, but do not expect passage reordering to buy accuracy.
...
- **Documentation inconsistencies — residual findings (LUC-140 follow-up)** — Second pass over the same defect class after an adversarial review of PR #238. User-facing docs: `introduction.md` and `commands/query.md` still told users to run `sprach -d` for troubleshooting (prose, not code blocks — the first sensor only scanned lines starting with `sprach `, a structural false negative); all five per-subcommand option tables (`ocr`, `summarize`, `query`, `translate`, `vision`) listed `-v`/`-vv` as subcommand flags although no subcommand accepts them (verbosity is top-level-only: `sprach -v ocr ...`, not `sprach ocr -v ...`); `commands/ocr.md` and `pipelines.md` had examples using the rejected form. Stale markers: `SMOKE_TEST.md` still said "schema v14" (SCHEMA_VERSION is 15); `implementation-status.md` attributed LUC-141 to v0.45.0 although it postdates the tag and sits under `[Unreleased]`; `IMPLEMENTATION.md` carried a "Multi-Backend Positioning" parenthetical left over from the v0.43.0 line; `roadmap.md` listed v0.45.0 under "Upcoming Release" while its own heading said "Released". Shipped help text: `sprach --list` advertised `-m lfm` and the `-m` help called `lfm` "the default" — `lfm` is not a valid model (it errors with "Unknown model 'lfm'") and `DEFAULT_MODEL` is `qwen3.5:4b`; the built-in `document-processing` skill's corrected `ocr_mode` list omitted `"figure"` (four variants exist: text/table/figure/formula). The `tests/docs_consistency.rs` sensor grew to seven checks, each validated by re-injecting the defect it guards — including one whose first version scanned the wrong command's output (`--help` instead of `--list`) and therefore could never fire.

- **Documentation-vs-reality inconsistencies found during onboarding (LUC-140, ex gh#228)** — Corrected the seven documented commands/flags that did not match the CLI, plus three further inconsistencies found while verifying. (1) `quickstart.md` and its Quick Tips told users to run `sprach -d` for debug, but `-d` does not exist — the flag is `-v` (with `-vv` for trace). (2) `--list-models` appeared in `configuration.md` and, more importantly, in the generated `SAMPLE_CONFIG` (`src/settings.rs`) that every new user reads — the real flag is `--list`. (3) `quickstart.md` presented `qwen3.5:4b` as "(default)", but the default is config-dependent; now labelled as an example. (4) `translate.md` claimed the colon was required for source auto-detection (`:pt`), while `sprach translate pt "text"` works — the source is optional. (5) A flag-ordering note was added to `chat.md` and `quickstart.md`: global flags (`--plain`, `-m`, `-t`, `-v`, `--soulless`) must precede the subcommand; `sprach chat --plain` is rejected by the parser. (6) The in-session `/doc import` help advertised `PDF, EPUB` support although `detect_file_type()` rejects both (users must extract via `run_command` first) — the help now also lists the real `--nowait` flag. (7) `vision.md` showed custom prompts without the `--` separator, which makes the prompt be parsed as another filename; all examples now use `--` and the requirement is explained in the Arguments table. Further fixes in the same PR, all found by verifying rather than trusting the issue text: `chat.md` still described chat as rendering at a **fixed 80 columns** (removed by the W6 Responsive Chat Rebuild in v0.44.0); the `content/document.rs` module docs listed PDF/EPUB as supported types; `IMPLEMENTATION.md`, `roadmap.md` and `implementation-status.md` still reported v0.43.0/v0.44.0 and schema v14 after the v0.45.0 release and schema v15; `-d` was also documented in `troubleshooting.md` (5x), `tools.md`, `configuration.md` and `prompts.md`; the built-in `document-processing` skill called `spawn_ocr_agent` with 3 arguments while the tool takes 2 (`file_path`, `ocr_mode`), and advertised `vision <file.pdf>` although PDFs must go through `pdftoppm` first; `vision`'s `--model` help claimed a `moondream` default while its own `long_about` claimed `qwen3.5:4b` and the manual claimed `qwen3.5-4b` — the real default is the global `[model].default`; and `src/vision/cli.rs`/`error.rs` still printed the pre-rename `ask vision` in `--help` and usage errors. All corrected commands were executed against the release binary. A `tests/docs_consistency.rs` sensor now guards this class (see the Testing section).

- **Imported documents invisible to `/search` (LUC-141, ex gh#233)** — Documents imported via `/doc import` are stored project-scoped (`conversation_id = NULL`, by design — documents belong to the project, not the session), but `/search` filtered results to the current session's `conversation_id` and additionally hardcoded `content_type = message` in the hybrid search path, so imported documents (and project-scoped notes) silently returned "No results found" — two independent filters had to be lifted. Fixes: (1) `/search` (`run_search`) now searches **all content types** (messages, notes, documents) via `search_content_hybrid` with `content_type: None`; (2) the keyword and semantic search paths in `src/content/db.rs` use a coupled conversation/project scope — session messages are always included, and NULL-conversation (`project-scoped`) rows match only when they belong to the **current project**, so other projects' content never leaks (conversation-only callers keep legacy semantics); (3) anonymous sessions get a clear error ("Cannot search in anonymous sessions. Restart chat without --anonymous.") instead of a misleading "No results found"; (4) results are labeled by content type via the centralized role labels — messages keep role labels (👤 User etc.), documents show 📄 Document, notes show 📝 Note, and the footer shows `project-scoped` instead of `unknown` for NULL-conversation rows. Globally-imported documents (`/doc import --global`) are not returned in project sessions — use the `remember` tool for global search (also unchanged: it searches with `conversation_id: None`). Auto-retrieval (context injection) remains message-only. Also strengthens SMOKE_TEST.md section 4, which passed vacuously (checked "no crash" rather than actual search results).
...
- **Embedding provider resolution in /search and /reindex (Issue #106)** — The `/search` and `/reindex` commands were using `state.provider` (the chat model's provider) to generate embeddings, instead of the embedding model's own provider resolved from `models.toml`. This caused failures when the chat model and embedding model were on different providers (e.g., chat on Ollama, embeddings on llama-swap). The `query` subcommand had the same issue. All three call sites now resolve the embedding-specific provider via `resolve_indexing_model()` and create a separate `OpenAICompatibleProvider` for embedding calls, matching the pattern already used in `repl.rs` (chat startup).

- **Removed proactive context-exceeded estimation (Issue #106)** — The `is_likely_context_exceeded()` function used a hardcoded `CHARS_PER_TOKEN=2.0` ratio to pre-reject text before sending it to the embedding server. This heuristic was nomic-specific and wrong for other models (CJK, code, multilingual). Removed the proactive check — the server's context-exceeded error is now the single source of truth, and is handled by the existing fallback chunking pipeline. Also removed `EMBEDDING_PREFIX_TOKENS`, `CONTEXT_SAFETY_MARGIN`, `estimate_tokens()`, and `is_likely_context_exceeded()` from `client.rs`.

- **Threaded context_length from config to all embedding call sites (Issue #106)** — All embedding calls in `session.rs`, `command_handlers.rs`, and `documents.rs` were passing `DEFAULT_CONTEXT_LENGTH` (512) hardcoded to the fallback chunking functions, ignoring the configured `num_ctx` from `models.toml`. Now they call `EmbeddingClient::context_length()` which returns the configured value.
