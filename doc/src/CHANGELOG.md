...
- **Retry Threshold with Backoff — Recoverable Server Errors (Issue #116)** — Foundation for retry infrastructure in the W2 Provider Chain (#116→#123). New `src/retry.rs` module defines `RetryCategory` (`ImmediateRetry`, `NetworkRetry`, `ServerRetry`, `RateLimitRetry`, `NoRetry`) with per-category max attempts and `retry_delay()` for backoff calculation. `RateLimitRetry` variant is pre-emptive support for #122 (OpenAI-compatible provider) — `Retry-After` header support will be wired in #122. **What works in this PR (verified by manual test Scenario 1):** Tool execution failures (`InternalToolError`) are now **recoverable** — the error message is pushed to the conversation as a tool result via `src/chat/recovery.rs` wrapper, allowing the LLM to self-correct within the same turn instead of aborting the entire conversation. `MAX_RETRIES`-based loop with `is_ollama_error_recoverable()` is replaced by `classify_for_retry(&OllamaError) → RetryCategory` with `category.is_retryable() && attempts < category.max_attempts()`. Cancel-aware sleep (`sleep_or_cancel()`) wraps `tokio::time::sleep` in `tokio::select!` with the cancel token, so Ctrl+C aborts backoff immediately. `is_ollama_error_recoverable()` is deprecated (will be removed in #119). **Startup health check (NEW):** New `capabilities::check_server_health()` calls `list_local_models()` with a 3-second timeout. If Ollama is unreachable, the app exits immediately with a clear error message ("Failed to reach Ollama server. Make sure Ollama is running") instead of hanging indefinitely during DB initialization. **Known limitations (resolved by #120):** Scenarios 2, 3, and 4 from the manual test (Server 500 with linear backoff, Network timeout with exponential backoff, Cancel-aware sleep during HTTP-layer errors) are NOT fully mitigated by this PR. The retry infrastructure is correctly designed and the loops are in place, but the `ollama-rs` HTTP client does not surface these errors when Ollama hangs (kill -STOP, packet drop, server stopped) — it just hangs. When #120 (OllamaProvider reqwest direct) lands, these scenarios become acceptance criteria. See `MANUAL_TEST_116.md` (in `~/`) and the W2 Wave Context in `IMPLEMENTATION.md`. W2 dependency: this is the first card in the W2 Provider Chain, prerequisite for #118, #119, #120, #121, #122, #123, #11.
