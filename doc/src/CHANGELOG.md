...
- **`/gc` command for database garbage collection (Issue #185)** — New `/gc` command identifies and removes database artifacts: empty assistant messages from cancelled LLM calls, orphaned chunks, and other inconsistencies. Shows what was found before cleaning, ensuring user visibility.
