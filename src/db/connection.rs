...
impl Database {
    ...
    /// Ensure the vec0 tables match the configured embedding dimensions.
    ///
    /// Reads the stored dimensions from `schema_meta`. If they differ
    /// from `dimensions`, DROP+reCREATE all 3 vec0 tables with
    /// `FLOAT[<dimensions>]` and reset `has_embedding` flags so the
    /// background recovery pipeline regenerates all embeddings.
    ///
    /// For new databases (schema_meta just created by migration), the
    /// stored value is '256' (the pre-v15 default). If the configured
    /// dimensions are also 256, this is a no-op.
    ///
    /// # Arguments
    ...
    pub fn ensure_vec0_dimensions(&self, dimensions: u32) -> Result<()> {
        ...
        let mut conn = self.conn.lock().expect("lock poisoned: db connection");
        ...
        if stored_dims == dimensions {
            log::debug!(
                "vec0 dimensions match ({}), no recreation needed",
                dimensions
            );
            return Ok(());
        }
        ...
        log::info!(
            "Embedding dimensions changed ({} → {}), recreating vec0 tables...",
            stored_dims,
            dimensions
        );

        // Wrap DROP+CREATE+UPDATE in a single transaction so that a mid-way
        // failure (process crash, IO error, malformed vec0 DDL) rolls back the
        // entire operation and leaves the DB in its prior consistent state.
        // Without this, each `execute_batch` commits independently and a crash
        // between the DROP and CREATE leaves the DB with no vec0 tables, or a
        // crash between CREATE and UPDATE leaves mixed dimensions + stale
        // `has_embedding` flags pointing at the old vectors.
        let tx = conn.transaction()?;

        // DROP all 3 vec0 tables
        tx.execute_batch("DROP TABLE IF EXISTS fact_embeddings;")?;
        tx.execute_batch("DROP TABLE IF EXISTS content_embeddings;")?;
        tx.execute_batch("DROP TABLE IF EXISTS chunk_embeddings_v2;")?;

        // reCREATE with the new dimensions
        let dim_str = format!("FLOAT[{}]", dimensions);
        tx.execute_batch(&format!(
            "CREATE VIRTUAL TABLE IF NOT EXISTS fact_embeddings USING vec0(
                fact_id INTEGER PRIMARY KEY,
                embedding {dim} distance_metric=cosine,
                +scope TEXT,
                +category TEXT,
                +project_id TEXT,
        ...
        tx.execute("UPDATE content_items SET has_embedding = 0", [])?;
        tx.execute("UPDATE content_chunks SET has_embedding = 0", [])?;

        // Update schema_meta
        tx.execute(
            "UPDATE schema_meta SET value = ?1 WHERE key = 'embedding_dims'",
        ...
        log::info!(
            "vec0 tables recreated with FLOAT[{}]. Embeddings will be regenerated by the background recovery pipeline.",
            dimensions
        );
    ...
    /// Migration v14 -> v15: Add schema_meta table for dynamic
    /// embedding dimensions tracking.
    ///
    /// The `schema_meta` table stores the current embedding
    /// dimensions so that `ensure_vec0_dimensions()` can detect
    /// when the user changes the configured dimensions and
    /// recreate the vec0 tables accordingly.
    fn migrate_v14_to_v15(conn: &Connection) -> Result<()> {
        ...
        // Record the current vec0 dimension (256 for all pre-v15
        // databases, since the schema hardcoded FLOAT[256]).
        conn.execute(
            "INSERT OR IGNORE INTO schema_meta (key, value) VALUES ('embedding_dims', '256');",
            [],
        )?;
        log::info!("Migration v14→v15: Added schema_meta table");
        Ok(())
    }
