...
impl CustomToolInfo {
    ...
    ///
    /// The bound is `crate::tools::Tool` (our own trait). Via the blanket
    /// impl in `src/tools/tool_trait.rs`, any `ollama_rs::Tool` also
...
impl<C: ChatHistory> CustomCoordinator<C> {
    ...
    /// The bound is `crate::tools::Tool` (our own trait). Via the blanket
    /// impl in `src/tools/tool_trait.rs`, any `ollama_rs::Tool` also
    /// satisfies this bound, so all existing `#[ollama_rs::function]`
    /// tools continue to work. Tools generated by `#[sprachspiel::tool]`
    /// satisfy both bounds via the dual-impl macro.
    pub fn add_tool<T: crate::tools::Tool + 'static>(mut self, tool: T) -> Self {
        self.tool_infos
            .push(CustomToolInfo::new::<T::Params, T>());
