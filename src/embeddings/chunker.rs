...
pub fn chunk_text_with_config(text: &str, config: &ChunkConfig) -> Vec<Chunk> {
    ...
    if chunks.len() >= 2 {
        let last_chunk = chunks.last().expect("chunks len >= 2 guarantees at least one chunk");
...
fn find_sentence_boundary(text: &str, target_pos: usize) -> usize {
    ...
    for pos in (search_start..target_pos).rev() {
        ...
        let ch = text[pos..].chars().next().expect("pos is a valid char boundary (checked above)");
        ...
        if ch == '.' || ch == '!' || ch == '?' || ch == '\n' {
            ...
            let next_ch = text[next_pos..].chars().next().expect("next_pos = pos + ch.len_utf8() is always a valid boundary");
